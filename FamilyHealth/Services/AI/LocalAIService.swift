...
final class LocalAIService: AIServiceProtocol {
        ...
        AsyncThrowingStream { continuation in
            Task {
                do {
                    ...
                    let systemPrompt = """
                    你是一个健康信息记录助手（非医疗专业人员）。你的任务是帮助用户整理和理解他们的健康数据记录，\
                    包括体检报告和病例记录。基于以下健康数据进行信息整理和说明：
                    ...
                    重要规则（必须严格遵守）：
                    1. 你不是医生，不具备医疗资质，严禁提供任何诊断结论或治疗方案
                    2. 仅客观整理和解释数据含义，不要做出疾病判断
                    3. 对于任何异常指标，建议用户咨询专业医生
                    4. 不推荐具体药物或治疗方式
                    5. 使用通俗易懂的语言解释专业术语
                    6. 如果数据不足以做出说明，请如实告知
                    7. 使用 Markdown 格式回复
    ...
    func analyze(
        ...
        let systemPrompt = """
        你是一个健康信息整理助手（非医疗专业人员）。请帮助用户整理以下体检报告内容，给出：
        1. **关键指标概览** — 列出主要指标及其参考范围
        2. **异常指标说明** — 客观说明超出参考范围的指标含义
        3. **生活方式参考** — 一般性的健康生活建议（非针对性治疗方案）
        4. **就医提醒** — 建议用户携带报告咨询专业医生
